...
//   author?:
//     | string
//     | { _id: string; username: string; profileImage?: string; fullName?: string };
//   likesCount?: number;
...
//     }
//     try {
//       await $api.post(`/likes/post/${post._id}`); // toggle
//       setLikesCount((p) => p + 1);
//     } catch (err) {
//       console.error('Ошибка при лайке поста:', err);
//     }
...
//       await $api.delete(`/posts/${post._id}`);
//       onUpdatePosts();
//       handleClose(); // закрываем через общий хэндлер
//       setShowDeleteConfirmation(false);
//     } catch (error) {
//       console.error('Ошибка при удалении поста:', error);
//     }
...
//               </span>
//               <div className={styles.nameCaption}>
//                 <span className={styles.user_name}>{author?.username || 'User'}</span>
//               </div>
//             </div>
...
//версию, что ниже, поменяла на эту, но она была без ошибок до проверки
// interface ModalPost {
//   _id: string;
//   image: string; // S3 URL
//   description?: string;
